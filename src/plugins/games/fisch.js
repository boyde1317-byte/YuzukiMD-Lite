...
import {
  ...
  GACHA_COST_COINS,
  GACHA_PITY_LIMIT,
} from "../../src/lib/yuzuki-fisch-data.js";
...
let thumbFish = null;
try {
  const p = path.join(process.cwd(), "src", "assets", "yuzuki", "yuzuki-fishit.jpg");
...
function ctx(title, body) {
  ...
  const sName = config.saluran?.name || config.bot?.name || "Yuzuki-AI";
  const c = {
